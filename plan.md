...
| DL-022 | 2026-07-19 | Close coverage gaps with deterministic behavioral tests without adding production-only test seams. | Scripted SQL failures and existing driver abstractions now prove reachable migration, lifecycle, settlement, uniqueness, retry, and shutdown boundaries. Fixed-structure JSON marshal failures, entropy-source failures, synthetic row-count corruption, and concrete broker failures remain uncovered where exercising them would require global hooks, production indirection, or external services. This preserves runtime design and test isolation while making the remaining coverage limits explicit. |
