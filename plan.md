...
Continue **M2-07** by pinning legacy SQL/source construction compatibility, moving the cohesive orchestration engine behind `internal/workflow`, removing root production imports of public `bus`, and replacing `bus` with a deprecated adapter over root plus the retained raw-runtime route. Then finish **M1-01/M1-04** with durable callback dispatch/recovery, Redis post-`Done` success evidence, and the explicit NATS product decision; keep the SQL batch lost-update race, partial batch fan-out, atomic chain advance, Redis claim/enqueue crash window, Core NATS ephemerality, SQS visibility extension, and RabbitMQ reconnect/context-aware lifecycle gaps explicit.
