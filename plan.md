...
- [ ] **M3-09 — Make schema migration configurable.** `DisableAutoMigrate` is now an additive opt-out while the established default remains enabled; startup is retryable after real SQLite DDL lock failure, and a real no-DDL test proves externally managed schema ownership. SQL processing fencing additively migrates a nullable generation-token column with an explicit quiescent worker-fleet rollout. Workflow-store auto-schema now selects executable SQLite, MySQL, and PostgreSQL key/payload types and its atomic transition contract runs on all three real dialects. Fresh MySQL keys use exact binary comparison and enforce their generated 255/512-byte capacities, while runtime validation discovers and preserves wider existing `VARBINARY` limits; every caller-managed non-`VARBINARY` workflow identity schema needs the explicit migration recorded in DL-019. Define the remaining versioned migration ownership/rollback policy and add concurrent migration and permission evidence for the queue tables, including the uniqueness expiry index and processing-token column.
...
| DL-019 | 2026-07-18 | Require `VARBINARY` for every MySQL workflow identity column and derive accepted widths from the connected schema. | This is a MySQL persisted-schema, runtime-behavior, and operational migration correction; it is not a source/API, configuration, wire-envelope, or minimum-Go-version break. Fresh auto-schema now works with byte-exact 255-byte workflow IDs and 512-byte callback keys, while wider existing `VARBINARY` columns retain their configured capacity. Managed `VARCHAR`, `TEXT`, and fixed-width `BINARY` columns now fail on first store use instead of silently conflating or padding identities. Before upgrade, quiesce workflow writers, audit comparison-equivalent keys for collisions, convert all seven key columns to compatible `VARBINARY` widths in one maintenance window, and then restart workers. |
