...
Complete **M2-05/M2-10** by removing the workflow envelope from ordinary direct jobs while preserving logical identity, middleware, retry, and observer behavior. Follow with **M5-07** fake consolidation and **M1-01/M1-04** durable callback/recovery evidence; keep the SQL batch lost-update race, partial batch fan-out, atomic chain advance, Redis claim/enqueue crash window, Core NATS ephemerality, SQS visibility extension, and RabbitMQ reconnect/context-aware lifecycle gaps explicit.
