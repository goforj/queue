...
- [ ] **M2-07 — Resolve the public `bus` direction.** Implement D-003 and D-010 in bounded slices: extract `internal/workflow`, remove root production imports of public `bus`, establish physical root models, then make `bus` a deprecated forwarding facade with source- and wire-compatibility fixtures. The internal protocol now owns schema version 1, all four physical delivery names, and envelope metadata decoding; literal protocol, transport-boundary, and legacy SQLite fixtures pin fallback behavior, JSON field order/casing, payload encoding, transport options, callback routes, all five table layouts, state mutation, idempotency, and pruning before engine ownership changes.
...
Continue **M2-07** by pinning external source/construction compatibility, moving the cohesive orchestration engine behind `internal/workflow`, removing root production imports of public `bus`, and replacing `bus` with a deprecated adapter over root plus the retained raw-runtime route. Then finish **M1-01/M1-04** with durable callback dispatch/recovery, Redis post-`Done` success evidence, and the explicit NATS product decision; keep the SQL batch lost-update race, partial batch fan-out, atomic chain advance, Redis claim/enqueue crash window, Core NATS ephemerality, SQS visibility extension, and RabbitMQ reconnect/context-aware lifecycle gaps explicit.
