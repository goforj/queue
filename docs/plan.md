...
Working branch: `refactor/remove-legacy-api`

## Goal

Make `queue` a coherent, dependable queue and workflow library with one normal application model, explicit ownership between delivery and orchestration, truthful backend guarantees, and validation that covers every module and supported deployment shape.
...
- Preserve compatibility by default, except for an explicitly approved breaking cleanup that removes the retired `bus`, `queuefake`, legacy observer aliases, and legacy configuration shims.
- Record every incompatibility in the decision log with its migration path.
...
- Keep `docs/plan.md` focused on decisions, executable work, evidence, and remaining risk. Move lengthy design specifications into dedicated documents and link them here.
...
- No public compatibility facade remains. Applications use the root `queue` package directly for dispatch, workflows, observation, stores, and fakes.
...
- The approved legacy cleanup removed the public `bus` and `queuefake` packages rather than retaining forwarding aliases. Applications now migrate directly to root `queue` APIs.
...
Decision: use one root `queue.Observer` and one root `queue.Event` superset. `queue.WithObserver` observes both delivery and workflow facts. The former root workflow observer aliases and the public `bus` event adapter were removed in the approved breaking cleanup, so there is no second event producer or observer contract.
...
Decision: the observer collapse is an intentional source/API boundary. `WithObserver` accepts the canonical root `Observer`, and one observer receives all layers. The legacy root workflow names and `bus` observer contract were removed because preserving their type identities would retain a second public event model.

Migration requirements:

- replace unkeyed event literals with keyed `queue.Event` literals because the event envelope gained correlation fields;
- adapt custom observer implementations with `queue.ObserverFunc` and configure an existing `*queue.Queue` with `queue.WithObserver` at construction;
...
Decision: root `queue` owns the canonical application model. The orchestration implementation lives behind `internal/workflow`, and root composes that engine directly. There is no public workflow-model or facade package.

The implementation was staged to avoid an import cycle while preserving one public model:
...
4. remove the retired facade, aliases, and configuration shims after root ownership and canonical coverage are established.

The extraction is configuration, persisted-data, runtime-behavior, operational, wire, and minimum-Go-version neutral. The final cleanup is a deliberate source/API break: migrate `bus` imports and configuration to root `queue`, `queuefake.New()` to `queue.NewFake()`, `Config.Observer` to `queue.WithObserver(...)`, and Redis `ServerLogger` to `DriverBaseConfig.Logger`.
...
- [x] **M1-06 — Repair public `UniqueFor`.** Public workflows use canonical logical identity across every backend; concurrent public SQL/Redis clients prove one backend-wide winner, restart persistence, and documented instance-versus-backend scope. SQL rolling upgrades atomically claim both the exact historical physical identity and canonical version-one identity with the queue row, so an outstanding matching historical or current lock rejects the dispatch and either collision rolls back the companion claim. Pre-version high-level envelopes carried volatile correlation, so their independently generated physical keys still require a longest-live-TTL producer cutover or an explicitly accepted transient duplicate window.
...
- [ ] **M1-14 — Restore backend readiness.** Readiness now survives the internal driver bridge, managed SQL checks the complete runtime table shape without DDL, and NATS supplies the deadline required for a real server roundtrip while preserving shorter caller deadlines. A public NATS negative uses a reachable non-NATS endpoint, and the live integration covers `Ready(context.Background())`. Remaining work includes negative unreachable tests for every external backend and composing worker-factory configuration into readiness, including invalid NATS default-subject detection and recovery after permanent producer reconnect exhaustion.
- [ ] **M1-15 — Replace wrapper-inflated capability checks.** Report actual capabilities independently of observers and adapters.
- [ ] **M1-16 — Make shutdown retryable and context-aware.** Root native/external lifecycles serialize startup with shutdown, latch drain intent before waiting on startup, retain partially started workers for cleanup or retry, keep one replaceable handler slot per backend registration, lease dispatch/readiness/control/admin operations before resources can close, retain state after failed cleanup, reject restart while draining, and close successfully exactly once. Native cleanup now drains workers without closing resources, waits the stable late-operation generation after every handler permit expires, and only then closes producer/storage resources. Redis owned resources close at most once, report joined close diagnostics to the caller that performs cleanup, and let a later root shutdown converge to terminal success instead of remaining permanently draining. Scoped continuation permits expire when their handler returns, while Sync and Workerpool reserve accepted delayed descendants and drain them within the caller's shutdown deadline. Replacement decorators retain runtime-owned continuation, settlement, provenance, attempt, and metadata state while keeping their own cancellation, deadlines, and user values. Context-unaware broker dialing and RabbitMQ resource closure, plus real broker deadline evidence, remain.
...
- [x] **M2-05 — Consolidate the Job model.** Root `queue.Job` is the sole public application specification, `queue.Message` is the delivered handler model, and `queue.StoredJob` is the persisted workflow model. Direct dispatch freezes exact root payload bytes instead of passing through the private fluent workflow DTO.
...
| DL-014 | 2026-07-18 | Keep canonical public workflow models physically owned by root `queue`, with private adapters around one `internal/workflow.Engine`; never expose an `internal` package as the real owner. | The temporary facade was removed by DL-030. Existing consumers migrate applicable names from `github.com/goforj/queue/bus` to `github.com/goforj/queue`. JSON/wire envelopes, SQL schemas/data, runtime outcomes, operational rollout, and the minimum Go version do not change. |
| DL-015 | 2026-07-18 | During migration, make the former facade a view of the root engine rather than a second engine. | Superseded by DL-030, which removes that facade. Applications configure the root queue directly. |
| DL-016 | 2026-07-18 | Dispatch ordinary root jobs by their application type and exact payload, carrying correlation in one versioned driver metadata record instead of the workflow envelope. | Wire behavior changes for root direct jobs, SQL adds nullable `queue_jobs.metadata_json`, and absent payload no longer becomes JSON `null`; arbitrary raw bytes reach handlers without a dispatch-time JSON re-marshal. New workers read old and new deliveries, but old workers cannot safely consume new direct types. Deploy new workers while producers retain legacy emission, switch producers only after all consumers are upgraded, and restore legacy emission plus drain direct backlog before worker rollback. Workflow envelopes, uniqueness keys, configuration files, and the minimum Go version remain stable. |
| DL-017 | 2026-07-18 | Make `queue.NewFake` the sole fake-state owner for direct and workflow records. | The temporary facade fakes were removed by DL-030. `queue.NewFake` preserves accepted-dispatch recording, builder policy, opaque workflow IDs, lookup/reset behavior, payload isolation, and concurrent access. No configuration, persisted-data, wire-format, operational-rollout, or minimum-Go-version change is implied. |
...
| DL-030 | 2026-07-20 | Remove the retired public compatibility surfaces instead of carrying aliases indefinitely. | This approved source/API break removes `bus`, `queuefake`, root workflow observer aliases, `Config.Observer`, and Redis `ServerLogger`. Migrate to root `queue`, `queue.NewFake`, `queue.WithObserver`, and `DriverBaseConfig.Logger`. No queue protocol, persisted workflow data, configuration-file format, or minimum Go version changes. |
...
| DL-024 | 2026-07-19 | Treat a Codecov upload as complete only after the exact workflow upload, commit report, and pull request comparison are processed. | The uploader returning success proves acceptance, not report freshness. Each run now has an explicit commit, build, and upload identity, and the coverage job polls Codecov with a bounded timeout before it can pass. Pull requests additionally require the exact base/head comparison. Fast contracts pin same-SHA aggregate sessions, pagination, stale and incomplete comparisons, transient API responses, and push behavior. The default badge remains intentionally tied to `main` until merge, and the mutable pull request comment remains eventually consistent. This changes CI evidence only; it does not change source/API, configuration, persisted data, runtime behavior, wire formats, operations, or the minimum Go version. |
| DL-025 | 2026-07-19 | Honor `WithHandlerContextDecorator` independently of whether an observer has recipients, while retaining Redis's single native execution boundary. | This corrects documented runtime behavior for Sync, Workerpool, SQL, NATS, SQS, and RabbitMQ without changing source/API, configuration, persisted data, wire formats, operational rollout, or the minimum Go version. Redis continues decorating exactly once in its worker, and a decorator that returns `nil` preserves the original context. |
| DL-026 | 2026-07-19 | Let already-drained Sync cleanup succeed with an expired context and track every admitted Sync dispatch in one mutex-protected work generation. | This corrects runtime lifecycle behavior without changing source/API, configuration, persisted data, wire formats, operational rollout, or the minimum Go version. Immediate and delayed work share a stable completion channel, pending work still honors caller cancellation, shutdown remains latched, and retries converge without waiter goroutines or late continuation admission. Workerpool performs the same continuation recheck while reserving its existing work generation. |
| DL-027 | 2026-07-19 | Make a multi-module release tag family dependency-complete and commit-consistent before any tag mutation, then publish it atomically. | This changes operational release tooling only. Real release validation and tag planning run from an archive of the exact captured commit, followed by final HEAD and cleanliness checks. Shared validation rejects malformed versions and release majors that do not match each module path. A release also fails closed for unresolved sibling pins, dependency-incomplete exclusions, dirty non-dry-run state, an unreadable Git status, stale existing tags, remote inspection failures, or a remote that cannot accept the complete family atomically. Release operators must commit synchronized sibling pins first, use `--allow-dirty` only for dry runs, and reuse existing tags only when they resolve to the captured release commit. There is no library source/API, configuration, persisted-data, runtime, wire-format, or minimum-Go-version change. |
| DL-028 | 2026-07-19 | Make externally managed queue readiness reject an absent or structurally incomplete schema. | `DisableAutoMigrate` still performs no queue DDL, but `Ready` and `StartWorkers` now fail when either operational object is not a base-table relation, including an ordinary or partitioned PostgreSQL table, or any runtime-used column is absent. Views are rejected because the runtime requires inserts, updates, deletes, transactions, and row locking. This intentionally tightens managed-mode runtime behavior without changing source/API, configuration shape, persisted data, wire formats, or the minimum Go version. Operators must provision the complete dialect-correct schema before declaring a deployment ready. The same runtime may retry after provisioning. Presence validation does not certify write permissions, exact SQL types, constraints, or performance indexes. |
| DL-029 | 2026-07-19 | Give every Core NATS server roundtrip a bounded context when the caller supplies no deadline. | The NATS client rejects `FlushWithContext` calls without a deadline, while public queue readiness accepts ordinary background contexts. Readiness, initial dispatch, and worker replacement publication now share a five-second bound and retain any shorter caller deadline. This corrects NATS runtime behavior without changing source/API, configuration, wire formats, persisted data, or the minimum Go version. Initial connection establishment remains context-unaware under M1-16. |
| DL-030 | 2026-07-20 | Preserve exact outstanding pre-version SQL `UniqueFor` claims by atomically acquiring historical and canonical identities. | Current SQL producers claim the unprefixed physical queue/type/payload digest and canonical version-one logical identity in the same transaction as the queue row. A collision or storage failure on either identity rolls back both claims and the row. This is a runtime and persisted-data compatibility correction with no source/API, configuration, schema, wire, or minimum-Go-version change. Each accepted unique dispatch stores two rows in the existing lock table; existing rows need no rewrite and normal expiry pruning removes both formats. Keep dual claims until all old SQL producers are gone and at least their longest possible `UniqueFor` duration has elapsed; this release retains them unconditionally. Historical high-level envelopes included generated correlation, so independently generated logical equivalents still require a longest-live-TTL cutover or an accepted transient duplicate window. The historical delimiter format can also preserve false collisions between distinct canonical jobs until retirement. |
| DL-031 | 2026-07-20 | Split native shutdown into worker drain, stable late-operation wait, and resource close, and preserve runtime delivery state across replacement context decorators. | Native backends stop delivery and drain accepted handlers without closing producer or storage resources. Root then waits the post-drain operation generation before final cleanup, so an admitted continuation cannot be overtaken. `PreserveDeliveryContext` is an additive advanced API that carries the source settlement, provenance, attempt, metadata, and expiring continuation permits into a replacement while retaining the replacement's cancellation, deadlines, and user values; source runtime facts override replacement facts. Redis snapshots Asynq retry and queue facts before decoration. This corrects runtime behavior without changing configuration, persisted data, wire formats, operational rollout, or the minimum Go version. |
| DL-032 | 2026-07-20 | Linearize handler registration with worker activation. | Native runtimes install the current logical handler generation before backend activation. External runtimes publish and catch up the constructed worker before activation. A non-nil registration completed while startup is in flight is therefore present on the potentially consuming backend, while stable handler slots and generation-owned ledgers retain one physical registration across replacements and failed-start retries. Nil and empty registrations remain no-ops and cannot erase pending handlers. This corrects runtime behavior without changing source/API, configuration, persisted data, wire formats, operational rollout, or the minimum Go version. |
| DL-033 | 2026-07-20 | Cache only successful workflow SQL schema initialization. | Schema DDL and MySQL key-limit discovery remain serialized, but caller cancellation, connectivity, locking, permission, and idempotent partial-DDL failures no longer poison the store instance permanently. A later operation retries the complete initialization sequence and publishes MySQL limits only after full success. Permanent failures are retried until operators repair the underlying condition, so repeated operations may produce repeated DDL or catalog attempts. This corrects runtime recovery behavior without changing source/API, configuration, persisted schema, wire formats, operational rollout, or the minimum Go version. |
| DL-034 | 2026-07-20 | Upgrade pgx to the first release that fixes GO-2026-5004 without raising unrelated module baselines. | `driver/postgresqueue` and the integration module pin pgx v5.9.2 or newer through a checked dependency policy. Because pgx v5.9.2 requires Go 1.25, the PostgreSQL driver, examples, integration tooling, and repository workspace now require Go 1.25. The root library and every non-PostgreSQL published driver remain on Go 1.24.4. An exact per-module policy prevents accidental baseline drift, while the workspace must match the highest module version. This is a minimum-Go-version incompatibility for PostgreSQL driver consumers and repository contributors using the workspace. It does not change queue source/API, configuration shape, persisted data, schema, or wire formats. It does incorporate pgx v5.9 runtime changes, including reduced prepared-statement protocol traffic, discarding pooled connections left in transactions during reset, and defaulting an omitted database user to the current operating-system user. PostgreSQL consumers must build with Go 1.25 or newer and should set the database user explicitly when they do not want that upstream default; consumers of the root or other driver modules need no migration. |
| DL-035 | 2026-07-20 | Keep one exhaustive public event catalog and emit archive facts only from confirmed terminal settlement owners. | The canonical `queue.Event` stream retains queue, worker, and workflow layers because backend acceptance, physical execution, and logical workflow state can diverge, while one observer and one envelope carry all three. Package-wide contract tests require every exported `EventKind` and every private workflow kind cast into the public stream to appear exactly once in the README and detailed contract with the runtime layer. `process_archived` now publishes only after a fenced SQL transition to `dead`. SQS deletion and RabbitMQ consumer acknowledgement remain the documented available settlement boundaries for positive processing facts, but neither proves irreversible archival, so SQS, RabbitMQ, Redis, NATS, and local runtimes omit the stronger archive fact. SQL stale-claim recovery remains a fenced bulk update and emits one identity-free `process_recovered` count fact per affected row. Queue-control calls are serialized with their facts, and collector pause state is an idempotent gauge. This adds truthful observer volume for terminal SQL failures without changing source/API, configuration, persisted data, wire formats, minimum Go versions, or settlement behavior. Consumers counting archive facts should expect the newly confirmed SQL events after upgrade. |

## Progress Log

### 2026-07-20

- Completed the unified event contract rather than leaving its reference as a partial list. README and detailed docs now cover all 32 exported kinds and their exact validation, settlement, callback, and support boundaries; an AST-backed parity test prevents catalog drift. SQL now emits `process_archived` only after a fenced transition to `dead`, while every built-in runtime that cannot prove irreversible archival omits it. Focused contracts cover archive presence and omission, and real SQL backend contracts prove confirmed archive delivery.
- Upgraded the PostgreSQL driver and integration suite to pgx v5.9.2, the first release that fixes GO-2026-5004. Added exact mixed-version module policy, overflow-safe semantic dependency floors across every direct owner, replacement rejection, and manifest-derived minimum-toolchain CI that compiles tagged integration, generated examples, and documentation tooling. The root and unrelated driver modules remain on Go 1.24.4, while the PostgreSQL-only Go 1.25 consumer requirement is explicit.
- Replaced one-shot workflow SQL schema failure caching with serialized success-only initialization. A deterministic SQLite regression starts the same store with a canceled first use, then proves a healthy retry completes migration and persists state; focused fix-only reviews verified MySQL limit publication, managed-schema continuity, idempotent DDL replay, and connection ownership.
- Closed the handler-registration startup gap for native and external runtimes. Deterministic race-enabled tests cover new and replaced types during live startup, multiple late types, context-derived handles, concurrent start callers, external factory delay, failed-start retry, and shutdown latching while proving one physical registration per type. Corrected the production guide to name the actual SQL driver recovery settings and regenerated the executed unit count to 969.
- Closed the exact-identity SQL rolling-producer uniqueness gap by claiming historical and canonical identities atomically with queue acceptance. Focused transaction fixtures cover both collision and failure phases for SQLite, PostgreSQL, and MySQL, while the shared real-dialect scenario proves outstanding, expired, rolled-back, and concurrent legacy/current transition behavior. Pre-version high-level envelopes retain the documented longest-live-TTL cutover because their generated correlation prevents exact historical matching.
- Split native shutdown into an explicit worker drain, stable post-drain lease wait, and resource close. Replacement handler contexts now retain runtime-owned continuation and settlement state without inheriting source cancellation or user values, and Redis snapshots Asynq transport facts before decoration.

### 2026-07-19

- Closed the three findings from the fourth fresh-context review. Handler context decoration no longer depends on observer configuration, idle Sync shutdown no longer loses to an already-canceled cleanup context, and the release command refuses unresolved sibling-module tag families before mutation.
- Closed the follow-up lifecycle and release-script edges found while cross-reviewing those fixes. Sync immediate and delayed jobs now share one locked work generation without waiter goroutines, escaped Sync and Workerpool continuations cannot reserve after their permit expires, exclusions preserve module dependency closure, malformed or path-incompatible versions are rejected, unreadable repository status fails closed, dirty trees cannot be tagged, existing local and remote tags must resolve to the captured release commit, remote lookup failures are distinct from absent tags, and the complete tag family is pushed atomically. Offline fixtures cover every failure before touching real repository tags.
- Closed the Codecov freshness race in which the GitHub coverage job could pass before Codecov merged the upload or processed the pull request comparison. Uploads now carry a unique run identity and the job waits for the exact SHA's aggregate commit totals and, on pull requests, the exact base/head comparison. Fast mock contracts cover reruns, pagination, stale or incomplete results, malformed responses, and push behavior.
...
- Audited every remaining changed-line coverage marker with fresh NATS, RabbitMQ, Redis, and documentation reviewers. Fixed-structure JSON errors, invalid injected wiring, entropy-source failure, concrete-client transport faults, generator process exits, and the bounded integration timeout remain explicit rather than adding global test hooks. The one deterministic production gap now has a real NATS startup test proving an invalid subscription leaves retryable lifecycle state; focused unit contracts also pin connection-drain diagnostic convergence, expired delivery readiness, and same-attempt uncommitted republish. The authoritative all-backend run passes and records 613 executed integration tests, and the complete 12-module test/vet plus two-pass generation gate remains clean.
...
- Added an explicit `DisableAutoMigrate` path across database wrappers, removed poisoned one-shot SQL startup, and proved startup can recover from a real SQLite schema lock. Managed readiness and startup now fail closed without mutating empty or incomplete queue schemas, remain retryable on the same runtime after external provisioning, and run canonical preprovisioned schemas through readiness, uniqueness, dispatch, and consumption on SQLite, MySQL, and PostgreSQL.
- Made Core NATS readiness usable through the public background-context contract by sharing the bounded server-roundtrip context already required by dispatch and worker publication. A public negative rejects a reachable non-NATS endpoint, and the live NATS readiness integration proves the exact background-context path.
...
- Rebuilt public `bus` as a deprecated compatibility facade before its removal under DL-030: option-free `bus.New(existingQueue)` shared the configured root engine, the raw-`busruntime.Runtime` seam delegated to the same internal engine, and source fixtures retained its migration behavior.
...
- Completed M5-07 by moving all fake state into `queue.FakeQueue` and running fake workflows through the production engine without retaining non-executing closure callbacks in fake runtime state. Focused tests cover execution-time recording, validation and cancellation failures, builder reuse and policy, payload isolation, lookup/reset behavior, and concurrent access under the race detector.
