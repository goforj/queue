...
| Atomic workflow-store transitions across SQL dialects | `TestWorkflowStoreIntegration_SQLite`; `TestWorkflowStoreIntegration_MySQL`; `TestWorkflowStoreIntegration_MySQLManagedWideKeys`; `TestWorkflowStoreIntegration_MySQLRejectsNonVARBINARYKeys`; `TestWorkflowStoreIntegration_Postgres` (`integration/root/workflow_store_integration_test.go`), plus shared memory/SQLite concurrency contracts (`internal/workflow/store_contract_test.go`) |
