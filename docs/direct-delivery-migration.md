...
SQL producers have a separate `UniqueFor` rolling-upgrade boundary. Older releases stored an unprefixed digest of the physical queue, type, and payload, while this release uses the canonical version-one logical identity. A current SQL dispatch atomically claims both identities in the same transaction as its queue row. An outstanding exact historical lock or canonical lock rejects the dispatch, an expired historical lock is renewed, and any collision or storage failure rolls back both claims and the queue row. This protects mixed producer versions when their physical queue, type, and payload bytes match exactly; it does not relax the no-overlap rule for SQL worker generations.

Pre-version high-level workflow envelopes contained freshly generated correlation values. Independently dispatched equivalent work therefore does not share an exact historical key, even when its logical type and payload match. Stop those old producers and wait at least their longest live `UniqueFor` duration before starting new producers, or explicitly accept a transient logical duplicate window. Stable low-level producers whose physical identity is unchanged may overlap because both versions contend on the historical key.

The current release retains dual claims unconditionally. A future release may stop writing historical keys only after the last old SQL producer has been removed and at least the longest `UniqueFor` duration it could have written has elapsed. Operators do not need to delete existing lock rows; normal expiry pruning removes both formats. The historical delimiter format can conflate distinct canonical jobs, such as type `a` with payload `b:c` and type `a:b` with payload `c` on the same queue, so the transition conservatively retains those false collisions until historical claiming is retired. Canonical version-one identity provides stable logical uniqueness for new work.
...
Logical uniqueness remains on the version-one queue/type/payload identity. Direct and legacy-envelope forms therefore collide within the same declared backend scope, and dispatch/job correlation does not change the key. SQL's additional historical claim exists only to preserve outstanding pre-version locks during producer upgrades.
