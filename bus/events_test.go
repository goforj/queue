...
func TestUnknownCallbackKindEmitsCallbackFailed(t *testing.T) {
	...
	if err := q.Dispatch(queue.NewTask(internalTaskCallback).Payload(payload)); err == nil {
...
func TestCallbackMissingRequiredIDsEmitsCallbackFailed(t *testing.T) {
	...
	for i, payloadMap := range tests {
		if err := q.Dispatch(queue.NewTask(internalTaskCallback).Payload(payloadMap)); err == nil {
